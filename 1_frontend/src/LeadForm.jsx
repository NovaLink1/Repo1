...
const LeadForm = ({ onAddLead, onUpdateLead, initialData }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const leadData = {
      ...
      website,
      status,
      bewertung: parseInt(bewertung) || 0, // Sicherstellen, dass bewertung eine Zahl ist
      notizen,
    };

    if (initialData) {
      // Lead aktualisieren, wenn initialData existiert
      onUpdateLead(initialData.id, leadData); // Aufruf der Update-Funktion im Elternteil
    } else {
      // Neues Lead anlegen, wenn kein initialData vorhanden ist
      onAddLead(leadData); // Aufruf der Add-Funktion im Elternteil
    }
